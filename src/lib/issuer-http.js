  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/health") {
        sendJson(response, 200, { ok: true, service: "CKBuilder Issuer Portal", version: "10.0.1", network: config.APP_NETWORK, chainWriteMode: config.CHAIN_WRITE_MODE, signingKeysLoaded: true, htmlSupport: true, submissionAttachments: true }, requestId); return;
      }

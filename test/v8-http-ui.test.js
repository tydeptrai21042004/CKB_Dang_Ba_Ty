...
test("v8 UI presents agent services before generic Mission Control and includes Fiber quote + receipt surfaces", () => {
  ...
  assert.ok(html.indexOf('id="agent-economy-hub"') < html.indexOf('id="ckb-mission-control"')); assert.match(html, /Delegate work, simulate payment, keep a verifiable receipt/); assert.match(html, /dry_run=true/);
});

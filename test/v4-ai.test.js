...
test("v4 Gemini request uses x-goog-api-key and generateContent",()=>{const p={id:"gemini",name:"Google Gemini",kind:"gemini",endpoint:"https://generativelanguage.googleapis.com/v1beta/models",model:"gemini-3.5-flash"};const r=buildAiRequest(p,"secret","gemini-3.5-flash",[{role:"system",content:"system"},{role:"user",content:"hello"}],0.2);assert.match(r.url,/gemini-3.5-flash:generateContent$/);assert.equal(r.headers["x-goog-api-key"],"secret");assert.equal(r.body.system_instruction.parts[0].text,"system");assert.equal(r.body.contents[0].parts[0].text,"hello");});
...
test("v4 Gemini call can be tested without network and does not persist key",async()=>{let captured;const fetchImpl=async(url,options)=>{captured={url,options};return {ok:true,status:200,json:async()=>({candidates:[{content:{parts:[{text:"answer"}]}}]})}};const r=await callOptionalAi({headers:{"x-ai-provider":"gemini","x-ai-api-key":"user-key","x-ai-model":"gemini-3.5-flash"},messages:[{role:"user",content:"hello"}],fetchImpl});assert.equal(r.text,"answer");assert.equal(captured.options.headers["x-goog-api-key"],"user-key");assert.equal(JSON.stringify(r).includes("user-key"),false);});
